...
if not(opts.fit_load_quadratic is None):
    ...
    indx_ok = np.logical_and(Y[indx_ok]>np.max(Y)*0.1,indx_ok)  # don't fit to points below 10% of peak value
    indx_ok = np.logical_and(Y[indx_ok]>2,indx_ok)  # don't fit to points below 2 absolute scale
    X=X[indx_ok]
    Y=Y[indx_ok] - lnL_shift
    Y_err = Y_err[indx_ok]
    dat_out_low_level_coord_names =     dat_out_low_level_coord_names[indx_ok]
