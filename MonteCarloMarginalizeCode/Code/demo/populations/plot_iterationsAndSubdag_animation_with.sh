...
plot_up_to() {
 ...
 rm -rf comp_so_far; 
 for id in `seq 0 ${n_it_max_before}`; do cat consolidated_${id}.composite ; done | sort -rg -k10 >> comp_so_far
...
plot_up_to_after() {
 ...
 fnames_subdag=`ls iteration_${it_subdag}_cip/posterior_samples*.dat | sort -n `
 fnames_here=`ls posterior_samples*.dat | sort -n | head -n ${n_it_max} |  tail -n ${size_end}`
  ...
  rm -rf comp_so_far;
 for id in `seq 0 ${n_it_max}`; do cat consolidated_${id}.composite  ; done | sort -rg -k10 >> comp_so_far
...
# fnames_old=`ls posterior_samples*.dat | sort -n | head -n ${n_it_max}`
 ...
 rm -rf comp_so_far;
 echo SUBDAG CONSOL ${it_subdag} ${n_it_max_before} ${n_it_max_before_subdag}
 for id in `seq 0 ${n_it_max_before}`; do cat consolidated_${id}.composite ; done  | sort -rg -k10 >> comp_so_far
 for id in `seq 0 ${n_it_max_before_subdag}`; do cat iteration_${it_subdag}_cip/consolidated_${id}.composite ; done | sort -rg -k10 >> comp_so_far
 echo SUBDAG CONSOL SIZE `wc -l comp_so_far`
 # plot just subdag 
...
do
  ...
  if [ ! -f "anim_${i}.png" ]; then 
    ${PLOT} --lnL-cut 15  `cat myargs.txt` --composite-file comp_so_far  --quantiles None --ci-list [0.9]
...
do
  ...
  if [ ! -e "anim_${IT_PLOT}.png" ]; then 
    echo COMPOSITE SIZE `wc -l comp_so_far`
