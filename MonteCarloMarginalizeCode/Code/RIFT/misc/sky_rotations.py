...
def rotate_sky_forwards_scalar(theta,phi):   # When theta=0 we are describing the coordinats of the zhat direction in the vecZ frame
    global frm
    return lalsimutils.polar_angles_in_frame_alt(frm, theta,phi)

def rotate_sky_forwards(th,ph):
    th_out= np.zeros(len(th))
    ph_out= np.zeros(len(th))
    for indx in np.arange(len(th)):
        th_out[indx],ph_out[indx] = rotate_sky_forwards_scalar(th[indx],ph[indx])
        return th_out, ph_out
    

def rotate_sky_backwards_scalar(theta,phi): # When theta=0, the vector should be along the vecZ direction and the polar angles returned its polar angles
    global frmInverse
    return lalsimutils.polar_angles_in_frame_alt(frmInverse, theta,phi)

def rotate_sky_backwards(th,ph):
    th_out= np.zeros(len(th))
    ph_out= np.zeros(len(th))
    for indx in np.arange(len(th)):
            th_out[indx],ph_out[indx] = rotate_sky_backwards_scalar(th[indx],ph[indx])
    return th_out, ph_out
