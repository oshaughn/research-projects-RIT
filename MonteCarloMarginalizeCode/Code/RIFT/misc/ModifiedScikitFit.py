...
class PolynomialFeatures(BaseEstimator, TransformerMixin):
    ...
    """
    def __init__(self, degree=2, interaction_only=False, include_bias=True,symmetry_list=None):
        self.degree = degree
    ...
    def _combinations(self,n_features, degree, interaction_only, include_bias):
        ...
        combos = chain.from_iterable(comb(range(n_features), i)
                                   for i in range(start, degree + 1))

        # Select items with only +1 symmetry
        combos_ok = []
        for line in combos:
#            print "Internal", line  # tuple
            signature = np.prod( np.array(self.symmetry_list)[list(line)] )
            if signature >=0:
                combos_ok.append(line)
#            signature = np.prod(np.power(np.array(self.symmetry_list)),line)
#            if signature >=0:
#                combos.append(line)
        return combos_ok
    ...
    def fit(self, X, y=None):
        ...
        combinations = self._combinations(self,n_features, self.degree,
                                          self.interaction_only,
                                          self.include_bias)
