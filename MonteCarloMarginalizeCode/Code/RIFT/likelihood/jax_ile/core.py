...
def _accumulate_unit(data, ra, dec, psi, incl, phiref, interp,
    ...
    for det in data.detector_names:
        ...
        # rho_sq is arrival-time independent here and legitimately so: the BASELINE model
        # has a static antenna response F (evaluated once at tref), so <h|h> does not
        # depend on where in the window the template is placed.  The slow-rotation model
        # does have that dependence -- see _accumulate_unit_banded, to which this function
        # dispatches whenever data.feature is set.
