...
def marginalized_likelihood(Q_lm, U_lm_pq, beta, lam, psi, inclination, phi_ref, distance, modes, reference_distance):
...
def precompute_terms_for_lisa(tref, t_window, hlms, hlms_conj, data_dict, psd_dict, flow, fNyq, fhigh, deltaT,  beta, lamda, analyticPSD_Q=False, inv_spec_trunc_Q=False, T_spec=0.):
   ...
   # TO precompute innerproducts, we need tref for time shift, t_window to select what time values we need 
    ...
    # calculate <d|h_lm>
    IP_time_series_A = lalsimutils.ComplexOverlap(flow, fhigh, fNyq, hlms[2,2].deltaF, psd_dict["A"], analyticPSD_Q, inv_spec_trunc_Q, T_spec, full_output =True)  # Incase the three arms have different PSDs. Assume all arms have same PSD for now and 2,2 mode is present
    ...
    for channel in ["A", "E", "T"]:
        ...
        for mode in hlms.keys():
            ...
            Q_lm[f"{channel}_{l}_{m}_xx"] = ((factored_likelihood.ComputeIPTimeSeries(inner_product, data_dict[channel], collect_mode_terms[channel][mode][0], -N_shift, N_window)))
            Q_lm[f"{channel}_{l}_{m}_xy"] = ((factored_likelihood.ComputeIPTimeSeries(inner_product, data_dict[channel], collect_mode_terms[channel][mode][1], -N_shift, N_window)))
            Q_lm[f"{channel}_{l}_{m}_xz"] = ((factored_likelihood.ComputeIPTimeSeries(inner_product, data_dict[channel], collect_mode_terms[channel][mode][2], -N_shift, N_window)))
            Q_lm[f"{channel}_{l}_{m}_yy"] = ((factored_likelihood.ComputeIPTimeSeries(inner_product, data_dict[channel], collect_mode_terms[channel][mode][3], -N_shift, N_window)))
            Q_lm[f"{channel}_{l}_{m}_yz"] = ((factored_likelihood.ComputeIPTimeSeries(inner_product, data_dict[channel], collect_mode_terms[channel][mode][4], -N_shift, N_window)))
            Q_lm[f"{channel}_{l}_{m}_zz"] = ((factored_likelihood.ComputeIPTimeSeries(inner_product, data_dict[channel], collect_mode_terms[channel][mode][5], -N_shift, N_window)))

    # calculate <h_lm|h_pq>
    U_lm_pq = {}
    IP_A = lalsimutils.ComplexIP(flow, fhigh, fNyq, hlms[2,2].deltaF, psd_dict["A"], analyticPSD_Q, inv_spec_trunc_Q, T_spec) # Incase the three arms have different PSDs. Assume all arms have same PSD for now and 2,2 mode is present
