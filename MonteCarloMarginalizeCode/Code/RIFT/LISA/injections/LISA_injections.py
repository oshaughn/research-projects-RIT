...
def calculate_snr(data_dict, fmin, fmax, fNyq, psd):
    ...
    IP_A = lsu.ComplexIP(fmin, fmax, fNyq, data_dict["A"].deltaF, psd["A"], False, False, 0.0,)
    IP_E = lsu.ComplexIP(fmin, fmax, fNyq, data_dict["A"].deltaF, psd["E"], False, False, 0.0,)
    IP_T = lsu.ComplexIP(fmin, fmax, fNyq, data_dict["A"].deltaF, psd["T"], False, False, 0.0,)
...
def generate_lisa_TDI_dict(param_dict):
    ...
    print(f"\nWaveform is being generated with m1 = {P.m1/lsu.lsu_MSUN}, m2 = {P.m2/lsu.lsu_MSUN}, s1z = {P.s1z}, s2z = {P.s2z}")
...
def generate_lisa_injections(data_dict, param_dict, calculate_snr = True):
