...
# jax is not in requirements.txt, so SKIP rather than let the ImportError escape at
...
try:
    ...
    if _pytest is None:
        raise
    _pytest.skip("jax_gp coordinates need jax: %s" % _exc, allow_module_level=True)
