...
def likelihood_function(phi, theta, tref, phiref, incl, psi, dist):
    ...
    tEnd = lal.GPSTimeNow()
    print " Evaluation time ", float(tMiddle- tStartEverything) + float(tMiddleBeforePlots2 -  tMiddleAfterPlots) + float(tEnd  - tMiddleAfterPlots2)
    plt.show()
    ...
    if rosSaveHighLikelihoodPoints:
        np.savetxt("points-singleifo.dat",(phi, theta, tref, phiref, incl, psi, dist,lnL))
...
if rosUseStrongPriorOnParameters:
    sampler.add_parameter("psi", functools.partial(mcsampler.gauss_samp, Psig.psi, 0.9), None, psi_min, psi_max)
    sampler.add_parameter("ra", functools.partial(mcsampler.gauss_samp, Psig.phi,0.9), None, ra_min, ra_max)
    sampler.add_parameter("dec", functools.partial(mcsampler.gauss_samp, Psig.theta,0.6), None, dec_min, dec_max)
    sampler.add_parameter("tref", functools.partial(mcsampler.gauss_samp, tEventFiducial, 0.005), None, tref_min, tref_max)
    sampler.add_parameter("phi", functools.partial(mcsampler.gauss_samp, Psig.phiref,0.6), None, phi_min, phi_max)
    sampler.add_parameter("incl", functools.partial(mcsampler.gauss_samp, Psig.incl,0.6), None, inc_min, inc_max)
    ...
    if rosUseTargetedDistance:
        sampler.add_parameter("dist", functools.partial(mcsampler.uniform_samp_vector, dist_min, distBoundGuess*1e6*lal.LAL_MSUN_SI ), None, dist_min, dist_max)
    else:
