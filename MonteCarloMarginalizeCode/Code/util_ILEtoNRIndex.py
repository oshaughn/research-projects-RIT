...
#
# GOAL
#    Accepts ILE default .dat output  and converts to NR index
#        indx m1 m2 s1x s1y s1z s2x s2y s2z XXX
#     -> -1 m1 m2 group param XXX
# 
# COMMENTS
#     - Does literal relabelling, *no maximization or averaging*
#
# EXAMPLE
#    python util_ILEtoNRIndex.py --group Sequence-RIT-Generic --fname  ~/g184098-FlipFlop.dat  | grep '^-1*'  > sample-nrindex.dat
#        + WARNING: Requires grep to strip lines not beginning with '-1'
...
#            print "    ... trying ", key
        ...
        if good_sim:
            if opts.verbose:
                print " Picked  ",key[0], key[1],  " with MOmega0 ", MOmega0 #, " and peak duration ", nrwf.internal_EstimatePeakL2M2Emission[key[0]][key[1]]
            matches = [good_sim]
    except:
        if opts.verbose:
            print " Backup plan: pick the first one "
        matches  = [matches[0]] # pick the first one.  Note we will want to reduce /downselect the lookup process

    if len(matches)>0 :
        print -1, line[1], line[2], matches[0][0], str(matches[0][1]).replace(' ',''),  " ".join(map( "{0}".format, line[9:]))
