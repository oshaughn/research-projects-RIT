...
do
echo SUBDAG EXTERNAL `echo $i | sed -e 's/\//_/g'` `pwd`/$i  DIR `echo $i | sed 's/marginalize_extrinsic_parameters.dag//g' | sed 's/marginalize_extrinsic_parameters_grid.dag//g' | sed 's/marginalize_intrinsic_parameters_BasicIterationWorkflow.dag//g' `
done > master.dag
