...
parser.add_argument("--integral-output", help="Output file for this set of parameters")
...
dat_mr = EOSManager.make_mr_lamba(my_eos.eos)  # r m(Msun) lambda
...
def lambda_tilde_here(delta):
    ...
    lam1 = lam_fit(m1)
    lam2 = lam_fit(m2)
    lambda_tilde, dLt = lalsimutils.tidal_lambda_tilde(m1,m2,lambda1,lambda2)
    return lambda_tilde
...
def likelihood_function(d):  
    ...
    if isinstance(d,float):
        return np.exp(my_fit([x,lam_here]))
    else:
        return np.exp(my_fit(np.c_[d,lam_here]))
