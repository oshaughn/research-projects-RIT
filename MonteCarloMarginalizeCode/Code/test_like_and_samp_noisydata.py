...
data_dict['H1']=lsu.frame_data_to_non_herm_hoff(fnameCache, "H1"+":FAKE-STRAIN")
data_dict['V1']=lsu.frame_data_to_non_herm_hoff(fnameCache, "V1"+":FAKE-STRAIN")
data_dict['L1']=lsu.frame_data_to_non_herm_hoff(fnameCache, "L1"+":FAKE-STRAIN")
...
for det in detectors:
    IP = lsu.ComplexIP(fLow=fminSNR, fNyq=fSample/2,deltaF=df,psd=psd_dict[det])
...
Psig = lsu.xml_to_ChooseWaveformParams_array("mdc.xml.gz")[0]  # Load in the physical parameters of the injection (=first element)
dfSig = Psig.deltaF = lsu.findDeltaF(Psig)  # NOT the full deltaf to avoid padding problems
Psig.print_params()
print " == Data report == "
rho2Net = 0
for det in detectors:
    Psig.detector = det
    hT = factored_likelihood.non_herm_hoff(Psig)
    fSampleSig = len(hT.data.data)*hT.deltaF
    IP = lsu.ComplexIP(fLow=fminSNR, fNyq=fSampleSig/2,deltaF=dfSig,psd=psd_dict[det])
    rhoExpected[det] = rhoDet = IP.norm(hT)
    rho2Net += rhoDet*rhoDet
    print det, rhoDet, "compare to result from raw data: ", rhoManual[det],  "; note it arrival time relative to fiducial of ", float(factored_likelihood.ComputeArrivalTimeAtDetector(det, Psig.phi,Psig.theta,Psig.tref) - theEpochFiducial)
...
P = lsu.ChooseWaveformParams(fmin=fminWavesTemplate, radec=False, incl=0.0,phiref=0.0, theta=0.0, phi=0,psi=0.0,
         m1=m1,m2=m2,
         ampO=ampO,
         fref=fref,
...
if checkInputs == True:
    ...
    print "  : Timing issues (checkme!) : fiducial = ", lsu.stringGPSNice(theEpochFiducial)

    print " ======= rholm test: Plot the lnLdata timeseries at the injection parameters (* STILL TIME OFFSET *)  =========="
    tmin = np.max(float(epoch_post - theEpochFiducial),factored_likelihood.tWindowReference[0]+0.03)   # the minimum time used is set by the rolling condition
