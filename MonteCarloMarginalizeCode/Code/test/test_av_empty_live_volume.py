...
Backport of the focused subset of the rift_O4d suite (PR #63) that covers the parts
of that fix carried onto this branch: the threshold clamp, the named error on an
empty live volume, the collapse verdict, and the ILE's cause attribution.
...
rho 51.4 -> 0/12, rho 72.1 -> 3%, rho 102.8 -> 5/12, rho 146.8 -> 11/12.
...
# built from one sample, indistinguishable in the output from a converged one.  A
# crashed export at least writes nothing and is visibly missing.
...
@pytest.mark.parametrize('n_live,ess,expect', [
    # (live samples, ESS) -> should the verdict call this collapsed?
    (1,    1.0,  True),    # rho 146.8: one sample exported
    (2,    1.7,  True),    # rho 146.8: slipped an earlier ESS<1.5-only rule
    (6,    3.0,  True),    # live set no larger than the dimension
    (4000, 16.3, False),   # rho 51.4, measured healthy (bottom of the healthy band)
    (4000, 34.3, False),   # rho 51.4, measured healthy (top of the healthy band)
    (4000, 4.0,  False),   # hard but not degenerate: low ESS, still many live points
])
def test_collapse_verdict_matches_the_measured_regimes(n_live, ess, expect):
    """Pin the decision boundary against the regimes measured on the real problem."""
    collapsed, reasons = live_volume_collapse_verdict(n_live, NDIM, ess=ess)
...
def test_ess_helper_matches_the_kish_definition():
    lw = np.log(np.array([1.0, 1.0, 1.0, 1.0]))
    assert ess_from_log_weights(lw) == pytest.approx(4.0)
    lw = np.log(np.array([1.0, 1e-12, 1e-12]))
    assert ess_from_log_weights(lw) == pytest.approx(1.0, abs=1e-6)
    # -inf entries (the high-SNR underflow) must be screened, not poison the reduction
    assert ess_from_log_weights(np.array([0.0, -np.inf, -np.inf])) == pytest.approx(1.0)
...
def test_known_gaussian_integral_is_recovered():
    ...
    # Tolerance is loose on purpose: AV carries a known few-tenths-of-a-nat bias on this
    # problem which is a property of the estimator, not of the collapse fix.  The purpose
    # of this test is to catch a gross regression in the integral, not to grade AV.
