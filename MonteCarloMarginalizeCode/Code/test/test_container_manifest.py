...
# ``dag_utils`` builds glue.pipeline CondorDAGJob objects, whose submit commands
# and universe are only reachable through accessors (the attributes themselves
# are name-mangled).  Keep the assertions below written against plain values.
def _cmds(job):
    return dict(job.get_condor_cmds())


def _universe(job):
    return job.get_universe()
...
def _make_ile_job(tmp_path, monkeypatch, singularity_image):
    ...
    Skips if the dag_utils backend cannot be imported in this env.
    """
...
def test_integration_container_universe(tmp_path, monkeypatch):
    ...
    assert _universe(job) == "container"          # HTCondor container universe
