...
for i in $(seq 1 $N); do
 for cfg in A B D AV GMM; do
  for arm in base cand; do
    [ "$arm" = base ] && code=$W/tier3-base/MonteCarloMarginalizeCode/Code || code=$W/rvs-naming/MonteCarloMarginalizeCode/Code
    o=$T/ens2/${cfg}_${arm}_$i; rm -rf $o; mkdir -p $o; cd $o
    t0=$SECONDS
...
print(','.join(repr(v) for v in a+g))")
 ...
 echo "replicate $i done ($(date +%H:%M:%S))" >> $T/ens2_progress.txt
done
echo "ENSEMBLE2 COMPLETE" >> $T/ens2_progress.txt
