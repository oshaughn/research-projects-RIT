...
\emph{Bottom panel}: The 90\% confidence interval derived from $L_{\rm red} p(\mc,\eta)$.  
 \textbf{Once OK with Ben Farr, add curves from MCMC TaylorF2}
}
\end{figure}
...
%
  By contrast,
our algorithm can be immediately applied to any existing time-domain model that provides $h_{lm}$,  at any mass,
including EOBNRv2HM \cite{gw-astro-EOBNR-Calibrated-2009} and SEOB \cite{gw-astro-EOBspin-Tarrachini2012}.  Minimal,
highly-compartmentalized  modifications are  needed in \texttt{lalsimulation} to implement $h_{lm}$ for most
...
% POINT: Comparison
Practically speaking, Reduced-order methods require precomputation Unlike reduced-order methods, : also 1h runtime; we
require no additional code, allowing runs \emph{immediately}  with \emph{any}
time-domain model, including SEOB and EOBNRv2HM; our runtime is duration independent and we see no evidence for needing more samples to account for
long duration at fixed SNR.
...
%For binary neutron star inspirals, the 
...
all \emph{other}  parameters are currently inefficiently explored.  
...
%
In the long term,  even though different strategies increase performance a comparable amount in a qualitatively similar way,
they do so with two different architectural paradigms: conventional serial MCMC with fast waveform evaluation versus
highly-parallelized Monte Carlo with slow waveforms and prefiltering.  
%
We expect  our algorithm can be accelerated.  
% POINT: Recognize
%
We hope  that by combining paradigms, future parameter estimation
strategies can reach extremely low latencies, ideally of order a few minutes, even in the advanced LIGO era. 
