...
%% \begin{align}
%% \ln L_{\rm data}(\lambda|t,\hat{n},\hat{k},\psi,d) &=  \sum_k\qmstateproduct{H_k}{\hat{H}_k}_k \nonumber \\
%% \label{eq:Implementation:lnLData}
%% &= (d_{\rm ref}/d) \text{Re} \sum_k \sum_{lm}(F_k(-\hat{k}) e^{-2\psi} \Y{-2}_{lm}(\hat{n}))^* Q_{k,lm}(t-\hat{k}\cdot x_k)
%% \end{align}
...
%% { U_{k,lm,l'm'}(\lambda)}& = \qmstateproduct{h_{lm}}{h_{l'm'}}_k \\
%% V_{k,lm,l'm'}(\lambda)& = \qmstateproduct{h_{lm}^*}{h_{l'm'}}_k \\
%% \ln L_{\rm model}(\lambda|\hat{n},\hat{k},\psi,d) &=
%%    -\frac{(d_{\rm ref}/d)^2}{2}\sum_k
%% \left[
%% {
%%  \frac{1}{2}|F_k(-\hat{k})|^2 U_{k,lm,lm'}(\lambda)[\Y{-2}_{lm}(\hat{n})]^*\Y{-2}_{l'm'}(\hat{n})
...
which we interpolate over $\lambda$ over some fixed subregion.

Then, we can evaluate $L(\lambda,\theta)$ for any $\lambda,\theta$ in that intrinsic $\times$ extrinsic hypercube. Do
Monte Carlo integration over the hypercube; parallelize over hypercubes.

This would let us cover much larger regions of paramter space, freeing us from the limitation of a few-mass-point grid,
and probably let us handle (slightly) higher dimensions.
