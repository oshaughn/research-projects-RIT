...
To be concrete, we evaluate the reduced likelihood $L_{\rm red}$ by integrating over sky position $\Omega$ represented
...
into $n_{\rm bins}$ bins $X_\alpha$, then evaluate a tempered, weighted histogram
\begin{eqnarray}
W_\alpha = \frac{\sum_{q} w_q^\beta \theta(x_q\in X_\alpha)}{\sum_q w_q^\beta}
\end{eqnarray}
where $\beta$ is a user-specified parameter to moderate the dynamic range of $w$.  
%
