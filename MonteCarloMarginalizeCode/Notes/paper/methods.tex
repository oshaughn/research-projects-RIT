...
In the next subsection we present a method for decomposing a waveform into intrinsic and extrinsic pieces.
...
h_{+,k}(t) &=& - \frac{2 M \eta}{D}\,v^2(\mc,\eta,t_k;t)\,\left[ \left( 1 + \cos^2 \iota \right)\,
	\cos 2 \left( \Phi(\mc,\eta,t_k;t) - \phi_c \right) + {\cal O}(v^3)\right]\ ,\\
h_{\times,k}(t) &=& - \frac{2 M \eta}{D}\,v^2(\mc,\eta,t_k;t)\,\left[ \left( 2 \cos \iota\right)\,
...
parameters appearing in the polarizations. In particular, we relate the polarizations to a set of $-2$ spin-weighted 
...
\end{equation}
where the the $\hat{h}_{lm}$ are the -2 spin-weighted spherical harmonics modes (see Ref. \editremark{ADDME})
