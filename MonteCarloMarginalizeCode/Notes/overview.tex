...
If the signal amplitude is sufficiently low ($ \rho^2/N_c \gtrsim 1$), these terms have a noticable impact on the
overlap.  That sets a mass vs SNR cut \editremark{provide explicitly}. 

%

\noindent \textbf{How is it useful?}: Massively reduces the complexity of $U,V$.
\end{shaded}
...
{\cal F}_+ &= F_+(\theta_S,\phi_S,\psi)\frac{ 1+\cos^2 \theta_E}{2} \\
{\cal F}_\times &= F_\times(\theta_S,\phi_S,\psi) \cos \theta_E \\
\end{align}
