...
\subsection{Extrinsic marginalization: Distance  [PLACEHOLDER DOCS] (*)}
...
%
For any \emph{individual} sky location and template, the actual distance posterior has the form \editremark{See eqna}
\begin{eqnarray}
...
\begin{shaded}
\noindent \textbf{Distance targeting with trigger [tempate-dependent sampling]}:  \editremark{XXX}
So in practice the distance distribution has the form
\begin{eqnarray}
p(r)dr \propto r^2 dr \exp[ -(r_0/r)^2/2 + (r_o/r)]
\end{eqnarray}
for some constant $r_o$ to be provided \editremark{provide a specific formula}.  An example of distance-targeted sampling is shown in Figure
...
\noindent \textbf{Time shift check}: \editremark{INTENTIONALLY BREAKING TIMESHIFT} by rolling the signal at the end.
