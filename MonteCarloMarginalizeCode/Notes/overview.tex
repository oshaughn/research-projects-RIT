...
\tilde{H}_k(f)_{\text{no time shift}} &= \frac{(d_{\rm ref}/d)}{2}[F_k h + F^*_k {\cal I} h]
  ...
  \right] \\
\ln L_{\rm model}(\lambda|\hat{n},\hat{k},\psi_J,d) &= -\frac{1}{2} \sum_k\qmstateproduct{H_k}{\hat{H}_k}_k 
 \\
&= -\frac{(d/d_{\rm ref})^2}{2}
...
This integral should require no trig operations !  (Maybe a few, to do $\psi_J$)  
...
\ln L_{\rm data}(\lambda|t,\hat{n},\hat{k},\psi_J,d)  = (d_{\rm ref}/d)[e^{+i2\psi_J} \bar{Q}(t)  + e^{+i2\psi_J}
  \bar{Q}(t)] 
\end{eqnarray}
Computing $\rho^2$ by just changing $\psi_J$ is pretty fast, if we store $\tilde{h}_{lm}(f)$.  At a minimum we need at
least one $O(N)$ evaluation (per evaluation of the easy extrinsic variable marginalization) to compute $\rho^2$ at
all...so repeating that 4 times isn't much of an added cost.  Assume we can therefore reproduce
\begin{eqnarray}
\ln L_{\rm model}  = -\frac{1}{2}\rho^2 + \Delta \rho^2 \cos 4(\psi-\psi_0)
\end{eqnarray}
