...
\editremark{Bugs}
 ...
 - overflow in exponential for likelihood evaluation!  This sets maximum SNR. \editremark{needs to be dealt with}
...
\subsubsection{Test suite 3: Full data-ingesting tool (single template)}
\noindent \textbf{Rationale}: A full data- and psd-ingesting tool, working from stored PSDs and \texttt{.cache} files,
provides a portable and immediately-usable tool for people to benchmark.

\nointext \textbf{Discrete PSD code}: The  code path using a discrete PSD had not yet been tested. [It is now, sort of.]
