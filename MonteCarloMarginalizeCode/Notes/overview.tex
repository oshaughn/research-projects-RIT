...
\item \textbf{Confirm normalization}: Chris' code provides a way to do integrals of the form $\int X dx$ via $p_s$ by
  evaluating $\int X/p_s  p_s dx$.  The likelihood evaluation code itself must hardcode the prior over angles and
  distance (trivial, but important).

As a result, we had better have $L_{\rm marg} = \int L p dx \le \text{max} L$ for real-data results.

\editremark{DOING IT THIS WAY MAKES DEBUGGING INCREDIBLY ANNOYING}. I want access to the low-level true $L$ in debugging
plots, not $L*p$.
...
\item \textbf{Run on cluster in vanilla mode}: Throw it at the clust
\end{itemize}

\editremark{TEST SAMPLING DISTRIBUTIONS THAT USE INFORMATION FROM THE DATA}
